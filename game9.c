...
int processEvent(SDL_Window *window, GameState *game)
{
	...
	const Uint8 *state = SDL_GetKeyboardState(NULL);
	    ...
	    if (game->enemies.x+40 <= game->girl.x+60 && game->enemies.x+40 >= game->girl.x)
	    { 

	    	if(game->enemies.y+40 <= game->girl.y+90 && game->enemies.y+40 >= game->girl.y)
	   	  		game->girl.alive=0;
	    }
